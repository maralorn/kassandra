...
import           Kassandra.Config        hiding ( backend
                                                , id
                                                )
...
backend = Backend
  { _backend_run          = \serve -> do
    ...
    concurrently_ (taskMonitor def (atomically . writeTChan broadCastChannel))
  ...
  BackendRouteSocket :/ (_, params) ->
    let mayUsername = join (params ^. at "username")
        ...
        action (Just (username, password))
          ...
          , PasswordCheckSuccess <- checkPassword (mkPassword password)
